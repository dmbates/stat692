...
Submit only the `.Rmd` file.  The file will be run through `knitr::knit` to verify that it produces the desired results.  Use `pdf_document` as the output format.  Align figures to the center of the line.  Allow them to float to the top or the bottom of the page.  Provide figure captions.

Load the `classroom` data frame that you saved in the previous set of exercises and attach the `ggplot2` package.

1. Provide a histogram of the `mathkind` (mathematics score in kindergarten) variable.  For this and all other plots in this assignment provide informative axis labels where appropriate.

2. Provide an empirical density plot of `mathkind`

```{r dens1,echo=FALSE,fig.align='center',fig.cap="Histogram of the kindergarten mathematics score",fig.height=2.5,fig.width=6.5,out.width="0.8\\textwidth",fig.pos="h",warning=FALSE,include=TRUE}
